// app.js - Système de Gestion des Agents - Version Complète Corrigée
...
let jokerConfig = {
    maxPerDay: 1,           // Nombre maximal de jokers pouvant travailler le même jour
    manualAssignment: true  // Si true, demande de choisir le joker lors d'une absence
};
...
// Données de test (version complète, inchangée)
function initializeTestData() {
    agents = [
        ...
        { code: 'L1A', nom: 'KAMOUN', prenom: 'Younes', groupe: 'A', tel: '06234563', adresse: 'Azerty', code_panique: '', poste: 'l1', cin: 'A709', date_naissance: '1973-11-05', matricule: 'S09278C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        { code: 'L4A', nom: 'BENTAIB', prenom: 'Younes', groupe: 'A', tel: '06274563', adresse: 'Azrty', code_panique: '', poste: 'l4', cin: 'A709', date_naissance: '1983-11-05', matricule: 'S09278C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        { code: 'I2A', nom: 'SADOUK', prenom: 'Yes', groupe: 'A', tel: '06734563', adresse: 'Azerty', code_panique: '', poste: 'I2', cin: 'A78809', date_naissance: '1983-11-05', matricule: 'S09678C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        { code: 'L9A', nom: 'MALOUANI', prenom: '...', groupe: 'A', tel: '06894563', adresse: 'Aerty', code_panique: '', poste: 'L9', cin: 'A788809', date_naissance: '1983-11-05', matricule: 'S09648C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        { code: 'Z2B', nom: 'KAROUCHE', prenom: 'Fayçal', groupe: 'B', tel: '', adresse: 'DP1400', code_panique: '', poste: 'Z2', cin: '743534', date_naissance: '', matricule: 'S13273C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        ...
        { code: 'L4B', nom: 'YOUNES', prenom: 'KHODAYRA', groupe: 'B', tel: '0696893480', adresse: '', code_panique: '842', poste: 'L4', cin: '660675', date_naissance: '1983-02-10', matricule: 'S09162C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        ...
        { code: 'Z2A', nom: 'TSOULI', prenom: 'ADIL', groupe: 'A', tel: '0767872200', adresse: '', code_panique: 'DP1400', poste: 'Z2A', cin: '414286', date_naissance: '2020-07-01', matricule: 'S09170C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
        ...
        { code: 'Z2C', nom: 'GRINEH', prenom: 'KHALID', groupe: 'C', tel: '', adresse: '', code_panique: 'DP1400', poste: 'Z2', cin: '670880', date_naissance: '1976-09-29', matricule: 'S09176C', date_entree: '2025-11-01', date_sortie: null, statut: 'actif' },
...
// ==================== PARTIE 2 : FONCTIONS UTILITAIRES ET GESTION DES SHIFTS ====================
...
// Shift théorique sans tenir compte des absences (pour joker)
...
// Retourne la liste des jokers disponibles pour une date donnée
...
// Demande à l'utilisateur de choisir un joker parmi ceux disponibles
...
// Trouve quel joker remplace un agent à une date donnée (pour affichage)
...
// Fonction principale de récupération du shift
...
// Compte les jours fériés effectifs
...
function calculateAgentStats(agentCode, month, year) {
    ...
    const totalConges = stats['C']+stats['M']+stats['A'];
    return [
        ...
        { label: 'Total général', value: totalTravailles + totalFeries + totalConges }
    ];
}

// Compte les jours fériés dans un mois
function countHolidaysInMonth(month, year) {
    let count = 0;
    const daysInMonth = new Date(year, month, 0).getDate();
    for (let day=1; day<=daysInMonth; day++) {
        if (isHolidayDate(new Date(year, month-1, day))) count++;
    }
    return count;
}

// Gestion des popups et notifications
...
function checkPassword() {
    if (prompt("🔐 Mot de passe:") === "Nabil1974") return true;
...
// ==================== PARTIE 3 : MENU PRINCIPAL ET SOUS-MENUS ====================
...
// ==================== PARTIE 4 : GESTION DES AGENTS (inchangée) ====================
// Pour gagner de la place, je reprends les fonctions essentielles. 
// En pratique, vous avez déjà ces fonctions dans votre fichier original.
...
// ==================== PARTIE 5 : GESTION DU PLANNING (AVEC MODIFICATIONS JOKERS) ====================
...
// ========== showGroupPlanning modifiée avec icônes ==========
...
// ========== showGlobalPlanning modifiée avec icônes ==========
...
// ========== showAgentPlanning modifiée avec icônes ==========
function showAgentPlanning(agentCode, month, year) {
    const agent = agents.find(a=>a.code===agentCode);
    if (!agent) { showSnackbar("⚠️ Agent non trouvé"); return; }
    const days = new Date(year, month, 0).getDate();
    let html = `<div class="info-section"><h3>Planning de ${agent.nom} ${agent.prenom}</h3><div style="overflow-x:auto"><table class="planning-table"><thead><tr><th>Date</th><th>Jour</th><th>Shift</th><th>Description</th><th>Actions</th></tr></thead><tbody>`;
    for (let d=1; d<=days; d++) {
        ...
        let title = '';
        let extraIcon = '';
        const existing = planningData[dateStr.substring(0,7)]?.[agentCode]?.[dateStr];
        ...
        html += `<tr>
            ...
            <td class="${isHolidayDate(date)?'holiday':''}">${JOURS_FRANCAIS[date.getDay()]}</td>
            <td style="background:${color};color:white;text-align:center" ${title}>${shift}${extraIcon}</td>
            <td>${SHIFT_LABELS[shift]||shift}</td>
            <td><button class="action-btn small" onclick="showShiftModification('${agentCode}','${dateStr}','${shift}')">✏️</button><button class="action-btn small red" onclick="showAbsenceFormForDate('${agentCode}','${dateStr}')">🚫</button></td>
        </tr>`;
    }
    html += `</tbody></table></div>`;
    const stats = calculateAgentStats(agentCode, month, year);
    html += `<div class="stats"><h4>Statistiques</h4><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:10px;">${stats.map(s=>`<div style="text-align:center"><div style="font-size:1.2em;font-weight:bold;color:#3498db">${s.value}</div><div>${s.label}</div></div>`).join('')}</div></div></div>`;
...
// ========== Fonctions de modification des shifts (inchangées) ==========
function showShiftModificationForm() {
    const html = `
        <div class="info-section">
            ...
            <div class="form-group"><label>Commentaire</label><textarea id="shiftComment" rows="3"></textarea></div>
        </div>
    `;
...
// ========== Fonctions d'absence avec gestion jokers (déjà modifiées) ==========
async function applyQuickAbsence(agentCode, dateStr) {
    const absenceType = document.getElementById('quickAbsenceType').value;
    ...
    if (jokerConfig.manualAssignment && ['C','M','A'].includes(absenceType)) {
        chosenJoker = await promptJokerChoice(agentCode, dateStr, absenceType);
    ...
    if (!planningData[monthKey]) planningData[monthKey] = {};
    if (!planningData[monthKey][agentCode]) planningData[monthKey][agentCode] = {};
    planningData[monthKey][agentCode][dateStr] = { shift: absenceType, type: 'absence', comment, recorded_at: new Date().toISOString() };
            ...
            <table style="width:100%; border-collapse:collapse;">
                <thead>
                    <tr style="background-color:#2c3e50;">
                        <th style="padding:8px; border:1px solid #555; text-align:left;">Type</th>
                        <th style="padding:8px; border:1px solid #555; text-align:center;">Nombre</th>
                    </tr>
                </thead>
                <tbody>
                    ${stats.map(s => `
                        <tr>
                            <td style="padding:6px; border:1px solid #555;">${s.label}</td>
                            <td style="padding:6px; border:1px solid #555; text-align:center; font-weight:bold;">${s.value}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
...
function showWorkedDaysMenu() {
    const html = `<div class="info-section"><h3>Jours Travaillés</h3><div class="form-group"><label>Période</label><select id="workedDaysPeriod"><option value="current_month">Ce mois</option><option value="last_month">Mois dernier</option><option value="current_quarter">Ce trimestre</option><option value="current_year">Cette année</option></select></div><div class="form-group"><label>Groupe</label><select id="workedDaysGroup"><option value="ALL">Tous</option>${[...new Set(agents.filter(a=>a.statut==='actif').map(a=>a.groupe))].sort().map(g=>`<option value="${g}">Groupe ${g}</option>`).join('')}</select></div></div>`;
...
function showWorkedDaysResults() {
    ...
    const results = filtered.map(agent => {
        let worked = 0, conges = 0, total = 0;
        let cur = new Date(start);
        while (cur <= end) {
            const dateStr = cur.toISOString().split('T')[0];
            const shift = getShiftForAgent(agent.code, dateStr);
            if (['1', '2', '3'].includes(shift)) worked++;
            if (['C', 'M', 'A'].includes(shift)) conges++;
            total++;
            cur.setDate(cur.getDate() + 1);
        }
        return { ...agent, worked, conges, total, rate: total ? ((worked / total) * 100).toFixed(1) : 0 };
    }).sort((a, b) => b.rate - a.rate);

    const html = `
        <div class="info-section">
            <h3>Jours Travaillés - ${group === 'ALL' ? 'Tous' : 'Groupe ' + group}</h3>
            <p>Période: ${periodText}</p>
            <div style="overflow-x:auto;">
                <table style="width:100%; border-collapse:collapse;">
                    <thead>
                        <tr style="background-color:#2c3e50;">
                            <th style="padding:8px; border:1px solid #555; text-align:center;">Rang</th>
                            <th style="padding:8px; border:1px solid #555; text-align:left;">Agent</th>
                            <th style="padding:8px; border:1px solid #555; text-align:center;">Groupe</th>
                            <th style="padding:8px; border:1px solid #555; text-align:center;">Travaillés</th>
                            <th style="padding:8px; border:1px solid #555; text-align:center;">Congés</th>
                            <th style="padding:8px; border:1px solid #555; text-align:center;">Taux</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${results.map((a, i) => `
                            <tr>
                                <td style="padding:6px; border:1px solid #555; text-align:center; font-weight:bold;" class="rank-${i+1}">${i+1}</td>
                                <td style="padding:6px; border:1px solid #555;">${a.nom} ${a.prenom}</td>
                                <td style="padding:6px; border:1px solid #555; text-align:center;">${a.groupe}</td>
                                <td style="padding:6px; border:1px solid #555; text-align:center;">${a.worked}/${a.total}</td>
                                <td style="padding:6px; border:1px solid #555; text-align:center;">${a.conges}</td>
                                <td style="padding:6px; border:1px solid #555; text-align:center; font-weight:bold;">${a.rate}%</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
    ...
    `;
    openPopup("📊 Jours Travaillés", html, `<button class="popup-button gray" onclick="displayStatisticsMenu()">Fermer</button>`);
}
...
function showSelectedGroupStats() {
    ...
    if (analysis === 'performance') {
        const html = `
            <div class="info-section">
                <h3>Performance ${group === 'ALL' ? 'Tous les groupes' : 'Groupe ' + group}</h3>
                <p>Période: ${start.toLocaleDateString()} - ${end.toLocaleDateString()}</p>
                <div style="overflow-x:auto;">
                    <table class="classement-table" style="width:100%; border-collapse:collapse;">
                        <thead>
                            <tr style="background-color:#34495e;">
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Rang</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:left;">Agent</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Groupe</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Travaillés</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Congés</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Maladie</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Fériés</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Total global</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${stats.map((s, i) => {
                                const totalGlobal = s.worked + s.conges + s.feries;
                                return `
                                    <tr>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center; font-weight:bold;" class="rank-${i + 1}">${i + 1}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50;">${s.agent.nom} ${s.agent.prenom}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.agent.groupe}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.worked}/${s.total}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.conges}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.sick}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.feries}</td>
                                        <td style="padding:6px; border:1px solid #2c3e50; text-align:center; font-weight:bold; background-color:#27ae60;">${totalGlobal}</td>
                                    </tr>
                                `;
                            }).join('')}
                        </tbody>
        ...
        const html = `
            <div class="info-section">
                <h3>Présence/Absence ${group === 'ALL' ? 'Tous les groupes' : 'Groupe ' + group}</h3>
                <p>Période: ${start.toLocaleDateString()} - ${end.toLocaleDateString()}</p>
                <div style="overflow-x:auto;">
                    <table class="classement-table" style="width:100%; border-collapse:collapse;">
                        <thead>
                            <tr style="background-color:#34495e;">
                                ...
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Congés</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Maladie</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Autre absence</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Taux présence</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${stats.map(s => `
                                <tr>
                                    ...
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.conges}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.sick}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.other}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center; font-weight:bold;">${s.rate}%</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
        ...
        const html = `
            <div class="info-section">
                <h3>Répartition des Shifts ${group === 'ALL' ? 'Tous les groupes' : 'Groupe ' + group}</h3>
                <p>Période: ${start.toLocaleDateString()} - ${end.toLocaleDateString()}</p>
                <div style="overflow-x:auto;">
                    <table class="classement-table" style="width:100%; border-collapse:collapse;">
                        <thead>
                            <tr style="background-color:#34495e;">
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:left;">Agent</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Groupe</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Matin (1)</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Après-midi (2)</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Nuit (3)</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Repos (R)</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Congés (C)</th>
                                <th style="padding:8px; border:1px solid #2c3e50; text-align:center;">Maladie (M)</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${stats.map(s => `
                                <tr>
                                    <td style="padding:6px; border:1px solid #2c3e50;">${s.agent.nom} ${s.agent.prenom}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.agent.groupe}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.shifts['1'] || 0}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.shifts['2'] || 0}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.shifts['3'] || 0}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.shifts['R'] || 0}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.shifts['C'] || 0}</td>
                                    <td style="padding:6px; border:1px solid #2c3e50; text-align:center;">${s.shifts['M'] || 0}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
...
// ==================== PARTIE 7 : CONGÉS & ABSENCES (avec gestion jokers pour périodes) ====================
...
function showAddLeaveForm() {
    const html = `
        <div class="info-section">
            ...
            <div id="periodLeaveSection" style="display:none"><div class="form-group"><label>Début</label><input type="date" id="periodStart"></div><div class="form-group"><label>Fin</label><input type="date" id="periodEnd"></div><div class="form-group"><label>Dimanches comme congés?</label><select id="sundayHandling"><option value="repos">Non</option><option value="conge">Oui</option></select></div></div>
...
// ========== saveLeave modifiée pour gérer les périodes avec jokers ==========
    ...
    if (leaveType === 'period') {
        ...
        for (let dateStr of dates) {
            ...
            if (jokerConfig.manualAssignment) {
                chosenJoker = await promptJokerChoice(agentCode, dateStr, 'C');
                if (chosenJoker === 'CANCEL') { closePopup(); return; }
            ...
            if (!planningData[monthKey]) planningData[monthKey] = {};
            if (!planningData[monthKey][agentCode]) planningData[monthKey][agentCode] = {};
            planningData[monthKey][agentCode][dateStr] = { shift: 'C', type: 'congé_période', period_id: leaveRecord.id, recorded_at: new Date().toISOString() };
        ...
        saveData();
        showSnackbar(`✅ Période enregistrée (${successCount}/${dates.length} jours avec remplacement)`);
    } else {
        // Ponctuel
...
// ========== deleteLeaveItem modifiée pour supprimer aussi les remplacements ==========
    ...
    if (isPeriod && periodId) {
        ...
        if (idx !== -1) {
            ...
            while (cur <= new Date(leave.end_date)) {
                ...
                // Supprimer les remplacements joker liés à cette période
        ...
        if (planningData[monthKey]?.[agentCode]?.[dateStr]) {
            ...
            // Supprimer tout remplacement joker pour cette date avec commentaire contenant l'agent
...
function generatePanicEditList() {
    return `<table class="classement-table"><thead><tr><th>Agent</th><th>Code</th><th>Action</th></tr></thead><tbody>${panicCodes.map(p=>{
        const agent = agents.find(a=>a.code===p.agent_code);
        return `<tr><td>${agent?agent.nom+' '+agent.prenom:p.agent_code}</td><td>${p.code}</td><td><button class="action-btn small blue" onclick="showEditPanicCode('${p.agent_code}')">✏️</button></td></tr>`;
    }).join('')}</tbody></table>`;
}
...
function showDeletePanicCodeList() {
    if (!panicCodes.length) { showSnackbar("ℹ️ Aucun code"); return; }
    const html = `<div class="info-section"><h3>🗑️ Supprimer Code</h3><table class="classement-table"><thead><tr><th>Agent</th><th>Code</th><th>Action</th></tr></thead><tbody>${panicCodes.map(p=>{
        const agent = agents.find(a=>a.code===p.agent_code);
        return `<tr><td>${agent?agent.nom+' '+agent.prenom:p.agent_code}</td><td>${p.code}</td><td><button class="action-btn small red" onclick="deletePanicCode('${p.agent_code}')">🗑️</button></td></tr>`;
...
function showPanicCodesList() {
    ...
    const html = `<div class="info-section"><h3>🚨 Liste des Codes</h3><table class="classement-table"><thead><tr><th>Agent</th><th>Code</th><th>Poste</th><th>Date</th><th>Actions</th></tr></thead><tbody>${panicCodes.map(p=>{
        const agent = agents.find(a=>a.code===p.agent_code);
        return `<tr><td>${agent?agent.nom+' '+agent.prenom:p.agent_code}</td><td><span style="color:#e74c3c">${p.code}</span></td><td>${p.poste||'-'}</td><td>${new Date(p.created_at).toLocaleDateString()}</td><td><button class="action-btn small blue" onclick="showEditPanicCode('${p.agent_code}')">✏️</button><button class="action-btn small red" onclick="deletePanicCode('${p.agent_code}')">🗑️</button></td></tr>`;
    }).join('')}</tbody></table></div>`;
...
function searchPanicCode() {
    ...
    const html = `<table class="classement-table"><thead><tr><th>Agent</th><th>Code</th><th>Poste</th><th>Date</th></tr></thead><tbody>${results.map(p=>{
        const agent = agents.find(a=>a.code===p.agent_code);
        return `<tr><td>${agent?agent.nom+' '+agent.prenom:p.agent_code}</td><td>${p.code}</td><td>${p.poste||'-'}</td><td>${new Date(p.created_at).toLocaleDateString()}</td></tr>`;
    }).join('')}</tbody></table>`;
...
// ==================== PARTIE 9 : GESTION RADIOS ====================
// (inchangée, identique à l'original, je l'inclus pour complétude mais sans modifications)
...
function saveNewUniform() {
    ...
    for (let item of items) {
        ...
        data[item] = { size, date, needsRenewal: new Date(date) < twoYearsAgo };
    }
    if (!ok) { showSnackbar("⚠️ Tous les champs obligatoires"); return false; }
...
function generateUniformEditList() {
    ...
    return `<table class="classement-table"><thead><tr><th>Agent</th>${items.map(i=>`<th>${i.charAt(0).toUpperCase()+i.slice(1)}</th>`).join('')}<th>Actions</th> </thead><tbody>${uniforms.map(u=>{
        return `<tr><td>${u.agentName}<br><small>${u.agentCode}</small>${items.map(i=>{
            const val = u[i];
...
function editUniform(agentCode) {
    ...
    setTimeout(()=>{
        ...
        ['chemise','pantalon','tricot','ceinture','chaussures','cravate','veste','parka'].forEach(item=>{
            if(document.getElementById(`uniform_${item}_size`)) document.getElementById(`uniform_${item}_size`).value = uniform[item].size;
            if(document.getElementById(`uniform_${item}_date`)) document.getElementById(`uniform_${item}_date`).value = uniform[item].date;
        });
...
function showUniformReport() {
    ...
    const needsRenewalCount = uniforms.filter(u=>{
        return ['chemise','pantalon','tricot','ceinture','chaussures','cravate','veste','parka'].some(i=>u[i].needsRenewal);
    }).length;
...
function showUniformStats() {
    ...
    const html = `<div class="info-section"><h3>📊 Statistiques Tailles</h3>${items.map(i=>`<div><strong>${i.charAt(0).toUpperCase()+i.slice(1)}s:</strong> ${Object.entries(stats[i]).map(([s,c])=>`${s}:${c}`).join(', ')}</div>`).join('')}</div>`;
...
function showUniformDeadlines() {
    ...
    const deadlines = uniforms.filter(u=>{
        return items.some(i=>{
            const renewal = new Date(u[i].date);
    ...
    const html = `<div class="info-section"><h3>📅 Échéances (90 jours)</h3>${deadlines.map(u=>{
        ...
        items.forEach(i=>{
            ...
            if (days <= 90) lines.push(`${i}: ${u[i].size} - ${days>0?days+' jours':'DÉPASSÉ'}`);
        });
...
function exportUniformReport() {
    if (!uniforms.length) { showSnackbar("ℹ️ Aucune donnée"); return; }
    const items = ['chemise','pantalon','tricot','ceinture','chaussures','cravate','veste','parka'];
    let csv = "Rapport Habillement\n\nAgent;Code;Groupe;"+items.map(i=>i.charAt(0).toUpperCase()+i.slice(1)+" Taille;"+i.charAt(0).toUpperCase()+i.slice(1)+" Date").join(';')+";Commentaires\n";
    uniforms.forEach(u=>{
        csv += `${u.agentName};${u.agentCode};${u.agentGroup};`+items.map(i=>`${u[i].size};${u[i].date}`).join(';')+`;"${u.comments||''}"\n`;
    });
    downloadCSV(csv, `Habillement_${new Date().toISOString().split('T')[0]}.csv`);
    showSnackbar("✅ Export terminé");
}

// ==================== PARTIE 11 : AVERTISSEMENTS ====================
// (inchangée, identique à l'original)
...
function showHolidaysList() {
    ...
    const html = `<div class="info-section"><h3>📋 Jours Fériés</h3><table class="classement-table"><thead><tr><th>Date</th><th>Description</th><th>Type</th><th>Actions</th> </thead><tbody>${holidays.map(h=>{
        ...
        return `<td>${fullDate}</td><td>${h.description}</td><td>${h.type}</td><td><button class="action-btn small blue" onclick="editHoliday('${h.date}')">✏️</button><button class="action-btn small red" onclick="deleteHoliday('${h.date}')">🗑️</button></td>`;
    }).join('')}</tbody></table></div>`;
...
function showHolidaysForYear() {
    ...
    const html = `<table class="classement-table"><thead><tr><th>Date</th><th>Jour</th><th>Description</th><th>Type</th> </thead><tbody>${filtered.map(h=>{
        ...
        return `<td>${year}-${h.date}</td><td>${dayName}</td><td>${h.description}</td><td>${h.type}</td>`;
    }).join('')}</tbody></tr>`;
    document.getElementById('holidaysYearDisplay').innerHTML = html;
}

// ==================== PARTIE 13 : EXPORTATIONS ====================
// (inchangée, identique à l'original)
...
// ==================== PARTIE 14 : CONFIGURATION (avec ajout Joker) ====================
...
// ========== MODIFICATION JOKER : Configuration des jokers ==========
